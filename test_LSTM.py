...
model = load_model("custom_LSTM.h5")  # Ensure the model loaded is the RNN model you trained.
...
def LSTM_based_prediction(input_seq, model, normalization_factor=100):
    ...
    if len(normalized_seq) == 3:
        model_input = np.array([normalized_seq[0], normalized_seq[1], normalized_seq[2]]).reshape(1, 3, 4)
    ...
    for frame, objects in test_dict.items():
        ...
        for obj in objects:
            ...
            for track_id, track_seq in track_memory.items():
                    prediction = LSTM_based_prediction(track_seq, model)
...
output_video_path = 'output_video_with_tracking_LSTM.webm'  # Path to save the new video
